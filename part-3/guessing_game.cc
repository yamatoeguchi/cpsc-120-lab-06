...
// Lab 06-03
//
// This is my guessing_game assignment
//
...
void ErrorMessage(const string& message) {
  ...
  cout << message << "\n";
  cout << "There was an error. Exiting the guessing game.\n";
}
...
int GatherGuess() {
  ...
  try {
    ...
    ErrorMessage("Error converting Player Input into Int.");
    exit(1);
  }
  return PlayerGuess;
}
...
void WarmerOrColder(int guess, int last_guess, int secret_number) {
  ...
  try {
    ...
    ErrorMessage("Error calculating if warmer or colder.");
    exit(1);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  // and tell the player if they are getting warmer or colder.h
  //
